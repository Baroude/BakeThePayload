...
class DiffHunk(BaseModel):
    ...
    removed_lines: List[Tuple[int, str]] = Field(default_factory=list, description="Removed lines with line numbers")
    added_lines: List[Tuple[int, str]] = Field(default_factory=list, description="Added lines with line numbers")
    function_context: Optional[str] = Field(default=None, description="Function or method context")
...
class UnifiedDiffParser:
    ...
    def _parse_hunk_content(self, lines: List[str], file_path: str, 
                           hunk_info: Tuple[int, int, int, int, str]) -> Tuple[DiffHunk, int]:
            ...
            for line_num, content in hunk.removed_lines:
                for pattern in self.security_patterns:
                    if pattern.regex.startswith('(-\\s*)'):
                        ...
                        if match:
                            context = self._build_context(hunk, line_num) if pattern.context_required else None
    ...
    def parse_and_analyze(self, diff_content: str) -> Dict[str, Union[List[DiffHunk], List[SecurityMatch]]]:
        ...
        return {
            ...
            "summary": {
                ...
                "high_confidence_issues": len([m for m in security_matches if m.confidence >= 0.8])
            }
        }