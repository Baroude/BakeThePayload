...
class VersionRange(BaseModel):
    ...
    def _check_constraint(
        self, version: Version, constraint: VersionConstraint
    ) -> bool:
    ...
    def _check_compatible(self, version: Version, constraint_version: Version) -> bool:
        ...
        if len(constraint_version.release) >= 2:
            return (
                version.release[0] == constraint_version.release[0]
                and version.release[1] == constraint_version.release[1]
            )
...
class VersionExtractor:
    ...
    def parse_version_constraint(
        self, constraint_str: str, ecosystem: EcosystemEnum
    ...
    def _parse_single_constraint(
        ...
        if range_match:
            ...
            constraint_type = constraint_type_map.get(
                operator, VersionConstraintType.GREATER_EQUAL
            )
            return VersionConstraint(
                constraint_type, version, constraint_str, ecosystem
            )
    ...
    def create_version_range(
        ...
        return VersionRange(
            constraints=constraints, ecosystem=ecosystem, original_string=constraint_str
        )
    ...
    def extract_versions_from_text(
        self, text: str, ecosystem: Optional[EcosystemEnum] = None
    ) -> List[str]:
