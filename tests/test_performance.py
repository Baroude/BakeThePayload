...
class TestModelPerformance:
    ...
    def test_vulnerability_report_creation_speed(
        ...
        ), f"Model creation took {result['time_seconds']:.3f}s, should be < 1s"
    ...
    def test_complex_model_creation(self, monitor: PerformanceMonitor) -> None:
        ...
        ), f"Complex model creation took {result['time_seconds']:.3f}s"
    ...
    def test_model_validation_performance(self, monitor: PerformanceMonitor) -> None:
        ...
        print(
            f"Validated 1000 models ({valid_count} valid, {error_count} errors) in {result['time_seconds']:.3f}s"
        )
...
class TestParserPerformance:
    ...
    def test_diff_parser_performance(self, monitor: PerformanceMonitor) -> None:
        ...
        assert (
            result["time_seconds"] < 2.0
        ), f"Diff parsing took {result['time_seconds']:.3f}s, should be < 2s"

        analysis_result = result["result"]
        assert len(analysis_result["hunks"]) > 0

        print(
            f"Parsed large diff ({len(large_diff)} chars) in {result['time_seconds']:.3f}s"
        )
    ...
    def test_advisory_parser_performance(self, monitor: PerformanceMonitor) -> None:
        ...
        def parse_multiple_advisories(count: int) -> List[VulnerabilityReport]:
            ...
            for i in range(count):
                ...
                advisory_data = {
                    ...
                    "details": f"This is a detailed description of vulnerability {i}. "
                    * 10,  # Make it longer
                    "severity": "HIGH",
                    "cvss": {
                        "score": 7.5,
        ...
        assert (
            result["time_seconds"] < 5.0
        )  # Should parse 100 advisories in under 5 seconds
    ...
    def test_version_extractor_performance(self, monitor: PerformanceMonitor) -> None:
            ...
            for i in range(count):
                ...
                try:
                    version_range = extractor.create_version_range(
                        f"{constraint_str}.{i % 10}", ecosystem  # Vary version
                    )
        ...
        print(
            f"Processed {len(result['result'])} version constraints in {result['time_seconds']:.3f}s"
        )
...
class TestMemoryUsage:
    ...
    def test_memory_usage_scaling(self, monitor: PerformanceMonitor) -> None:
        ...
        for size in sizes:
            ...
            print(
                f"Size: {size}, Memory: {result['peak_memory_mb']:.2f}MB, Per model: {memory_per_model:.4f}MB"
            )
