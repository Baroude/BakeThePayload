...
from analysis.context import (
    CodeContextExtractor, LanguageDetector, SupportedLanguage, 
    FunctionContext, CodeContext
)
from analysis.grammar import LanguageGrammarManager, GrammarStatus, detect_language_from_content
from analysis.callgraph import CallGraphBuilder, CallGraph, CallGraphNode
...
class TestLanguageDetector:
    ...
    def test_python_detection(self):
        ...
        assert LanguageDetector.detect_language("/path/to/script.py") == SupportedLanguage.PYTHON
    
    def test_javascript_detection(self):
        assert LanguageDetector.detect_language("app.js") == SupportedLanguage.JAVASCRIPT
        assert LanguageDetector.detect_language("component.jsx") == SupportedLanguage.JAVASCRIPT
    
    def test_typescript_detection(self):
        assert LanguageDetector.detect_language("app.ts") == SupportedLanguage.TYPESCRIPT
        assert LanguageDetector.detect_language("component.tsx") == SupportedLanguage.TYPESCRIPT
...
class TestLanguageGrammarManager:
    ...
    def test_language_statistics(self):
        ...
        assert stats['available'] + stats['unavailable'] + stats['error'] == stats['total_languages']
...
class TestContentBasedDetection:
    ...
    def test_java_content_detection(self):
        content = "package com.example;\npublic class Test {\n    public void main() {}\n}"
        lang = detect_language_from_content(content, "unknown")
        assert lang == SupportedLanguage.JAVA
    
    def test_ruby_content_detection(self):
        content = "def hello\n  puts 'world'\nend"
        lang = detect_language_from_content(content, "unknown")
...
"""
    ...
    def test_target_line_filtering(self):
        extractor = CodeContextExtractor()
...
"""
        ...
        context = extractor.extract_context("test.py", python_code, target_lines=[5, 6, 7])
        
        # Should primarily find the target function
        target_found = any(func.name == "target_function" for func in context.primary_functions)
        assert target_found
...
""")
            ...
            main_functions = [func for func in context.primary_functions if func.name == "main"]
...
class TestTreeSitterRealWorld:
    ...
    def test_ruby_jwe_vulnerability_pattern(self):
...
"""
        ...
        decrypt_found = any(func.name == "decrypt" for func in context.primary_functions)
