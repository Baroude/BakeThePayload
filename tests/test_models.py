...
class TestSecurityBaseModel:
    ...
    def test_metadata_field_constraints(self) -> None:
        ...
        complex_metadata = {
            "nested_dict": {
                "level1": {"level2": {"value": "deep_nested_value", "number": 42}}
            },
        ...
        assert (
            model.metadata["nested_dict"]["level1"]["level2"]["value"]
            == "deep_nested_value"
        )
    ...
    def test_model_serialization_edge_cases(self) -> None:
        ...
        challenging_metadata = {
            ...
            "deeply_nested": {
                "level1": {
                    "level2": {"level3": {"level4": {"level5": {"deep_value": True}}}}
                },
            },
        ...
        assert (
            json_data["metadata"]["unicode_test"]
            == "🔐 Security Test 中文 العربية ñáéíóú αβγδε"
        )
        assert (
            json_data["metadata"]["deeply_nested"]["level1"]["level2"]["level3"][
                "level4"
            ]["level5"]["deep_value"]
            is True
        )
        ...
        assert (
            parsed_back["metadata"]["unicode_test"]
            == "🔐 Security Test 中文 العربية ñáéíóú αβγδε"
        )
...
class TestEvidence:
    ...
    def test_evidence_with_extremely_long_content(self) -> None:
        ...
        evidence = Evidence(type="large_content", content=large_content, confidence=0.7)
    ...
    def test_evidence_with_special_file_paths(self) -> None:
        ...
        test_cases = [
            ...
            (
                "/very/long/path/with/many/nested/directories/and/subdirectories/file.py",
                "very_long_path",
            ),
    ...
    def test_evidence_confidence_edge_cases(self) -> None:
        ...
        evidence_close_to_zero = Evidence(
            type="test", content="test", confidence=0.0000001
        )
        assert evidence_close_to_zero.confidence == 0.0000001

        evidence_close_to_one = Evidence(
            type="test", content="test", confidence=0.9999999
        )
...
class TestVulnerabilityReport:
    ...
    def test_vulnerability_with_circular_reference_handling(self) -> None:
        ...
        evidence1 = Evidence(
            type="primary", content="Primary evidence content", confidence=0.8
        )

        evidence2 = Evidence(
            type="secondary", content="Secondary evidence content", confidence=0.7
        )
...
class TestExploitFlow:
    ...
    def test_exploit_flow_validation(self) -> None:
        ...
        edge = FlowEdge(
            edge_id=uuid4(), source=node1_id, target=node2_id, probability=0.8
        )
    ...
    def test_exploit_flow_with_circular_edge_references(self) -> None:
        ...
        self_referencing_edges = [
            e for e in circular_flow.edges if e.source == e.target
        ]
    ...
    def test_exploit_flow_with_disconnected_subgraphs(self) -> None:
        ...
        disconnected_flow = ExploitFlow(
            ...
            entry_points=[
                cluster1_entry.node_id,
                cluster2_auth.node_id,
            ],  # Multiple entry points
            impact_nodes=[
                cluster1_validation.node_id,
                cluster2_impact.node_id,
            ],  # Multiple impacts
        )
    ...
    def test_exploit_flow_validation_performance_large_graphs(self) -> None:
        ...
        assert (
            validation_time < 5.0
        ), f"Validation took {validation_time:.3f}s, should be < 5s"
        assert validation_result == True
        assert len(large_flow.nodes) == 1000
        assert (
            len(large_flow.edges) >= 1400
        )  # At least most of the generated edges (accounting for potential duplicates)
    ...
    def test_complex_exploit_flow_traversal_validation(self) -> None:
        ...
        processing_node = ExploitNode(
            node_id=processing_id,
            type=NodeType.DATA_PROCESSING,
            title="Processing",
        ...
        edges = [
            ...
            FlowEdge(
                edge_id=uuid4(), source=auth_id, target=validation_id, probability=0.7
            ),
            FlowEdge(
                edge_id=uuid4(),
                source=validation_id,
                target=processing_id,
                probability=0.9,
            ),
            FlowEdge(
                edge_id=uuid4(),
            ...
            FlowEdge(
                edge_id=uuid4(), source=entry_id, target=processing_id, probability=0.6
            ),
        ]
        ...
        assert (
            len(adjacency.get(entry_id, [])) >= 2
        )  # Entry has multiple outgoing edges
