...
class RepositoryManager:
    ...
    async def get_commit_history(
        ...
        try:
            ...
            all_commits_cmd = [
                ...
                "--oneline", "--pretty=format:%H|%s|%an|%ad",
                "--date=iso"
            ]
    ...
    async def filter_repository_files(self, repo_path: Path) -> float:
        ...
        # Patterns to remove
        remove_patterns = [
            "*.md", "*.txt", "*.pdf", "*.doc*",  # Documentation
            "test*/", "spec/", "*test.py", "*_test.*",  # Tests
            "*.exe", "*.dll", "*.so", "*.dylib",  # Binaries
            "*.jpg", "*.png", "*.gif", "*.svg",  # Images
            ".git/logs/", ".git/refs/remotes/",  # Git metadata
            "node_modules/", "__pycache__/", "*.pyc"  # Build artifacts
        ]
        ...
        try:
            ...
            if process.returncode != 0:
                ...
                if "not found" in error_msg.lower():
                    ...
                    raise RuntimeError(f"Network timeout cloning repository: {repo_url}")
                else:
    ...
    async def _remove_files_matching_pattern(self, repo_path: Path, pattern: str) -> None:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            async with session.get(api_url, headers=headers) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"GitHub API server error ({response.status}) for {repo_url}")
                    return 0.5  # Conservative estimate
                else:
                    logger.warning(f"Unexpected GitHub API response {response.status} for {repo_url}")
                    return 0.5
