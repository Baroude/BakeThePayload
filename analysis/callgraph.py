...
class CallGraphBuilder:
    ...
    def build_call_graph(self, file_paths: List[str], 
                        target_functions: Optional[List[str]] = None,
                        max_depth: int = 3) -> CallGraph:
        """
        ...
        for file_path, functions in function_contexts.items():
            for func in functions:
                ...
                for callee_name in func.calls_made:
                    ...
                    callee_file = self._find_function_file(callee_name, function_contexts)
    ...
    def _extract_all_functions(self, file_paths: List[str]) -> Dict[str, List[FunctionContext]]:
    ...
    def _find_function_file(self, function_name: str, 
                          function_contexts: Dict[str, List[FunctionContext]]) -> Optional[str]:
    ...
    def _calculate_graph_confidence(self, nodes: Dict[str, CallGraphNode],
                                  relationships: List[CallRelationship]) -> float:
        ...
        if total_calls > 0:
            resolved_calls = sum(1 for rel in relationships if rel.callee_file is not None)
            resolution_rate = resolved_calls / total_calls
            base_confidence += 0.3 * resolution_rate
        
        # Boost for high-confidence relationships
        if total_calls > 0:
            high_confidence_calls = sum(1 for rel in relationships if rel.confidence > 0.7)
            confidence_rate = high_confidence_calls / total_calls
            base_confidence += 0.1 * confidence_rate
        
        return min(base_confidence, 1.0)
    
    def get_function_callers(self, graph: CallGraph, function_name: str, 
                           file_path: str) -> List[CallGraphNode]:
    ...
    def get_function_callees(self, graph: CallGraph, function_name: str,
                           file_path: str) -> List[CallGraphNode]:
    ...
    def find_call_paths(self, graph: CallGraph, start_function: str, 
                       start_file: str, target_function: str,
        ...
        def dfs_path_search(current_key: str, target: str, path: List[str], depth: int):
            if depth > max_depth:
                return
    ...
    def get_graph_statistics(self, graph: CallGraph) -> Dict[str, any]:
        ...
        internal_calls = sum(1 for rel in graph.relationships if rel.callee_file is not None)
        external_calls = total_relationships - internal_calls
        
        # Find nodes with most connections
        max_calls_out = max((len(node.calls_out) for node in graph.nodes.values()), default=0)
        max_called_by = max((len(node.called_by) for node in graph.nodes.values()), default=0)
        ...
        return {
            ...
            'resolution_rate': internal_calls / total_relationships if total_relationships > 0 else 0,
