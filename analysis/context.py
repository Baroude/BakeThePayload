...
class StaticQueryManager:
    ...
    def _get_query_patterns(self, language: SupportedLanguage) -> Dict[str, str]:
        ...
        if language == SupportedLanguage.PYTHON:
            from .queries.python import (
                FUNCTION_DEFINITIONS, METHOD_DEFINITIONS, ALL_CALLS,
                CLASS_DEFINITIONS, ASSIGNMENTS, IMPORT_STATEMENTS
            )
            return {
            ...
            from .queries.javascript import (
                ALL_FUNCTIONS, ALL_CALLS, CLASS_DECLARATIONS,
                VARIABLE_DECLARATIONS, IMPORT_STATEMENTS
            )
            return {
            ...
            from .queries.java import (
                ALL_METHODS, METHOD_INVOCATIONS, ALL_TYPES,
                FIELD_DECLARATIONS, IMPORT_DECLARATIONS
            )
            return {
                'methods': ALL_METHODS,
            ...
            from .queries.ruby import (
                ALL_DEFINITIONS, ALL_CALLS, ALL_ASSIGNMENTS, REQUIRE_STATEMENTS
            )
            return {
    ...
    def execute_query(self, language: SupportedLanguage, content: str, 
            ...
            for pattern_index, captures_dict in matches:
                ...
                for capture_name, nodes in captures_dict.items():
                    for node in nodes:
                        ...
                        if main_node is None or (node.end_byte - node.start_byte) > (main_node.end_byte - main_node.start_byte):
                            main_node = node
                
                if main_node:
                    # Create a single result with all captures from this match
                    main_text = main_node.text.decode('utf-8') if hasattr(main_node.text, 'decode') else str(main_node.text)
                    ...
                    result = QueryResult(
                        ...
                        start_point=(main_node.start_point[0], main_node.start_point[1]),
...
class CodeContextExtractor:
    ...
    def extract_context(self, file_path: str, content: str, 
                       ...
                       vulnerability_type: Optional[str] = None) -> CodeContext:
        """
        ...
        class_definitions = query_results.get('classes', []) + query_results.get('types', [])
        import_statements = query_results.get('imports', []) + query_results.get('requires', [])
    ...
    def _execute_all_queries(self, language: SupportedLanguage, 
                           content: str) -> Dict[str, List[QueryResult]]:
    ...
    def _extract_primary_functions(self, query_results: Dict[str, List[QueryResult]],
                                 content: str, target_lines: Optional[List[int]]) -> List[FunctionContext]:
        ...
        for result in function_results:
            if result.name.endswith('.definition'):
                ...
                func_context = self._create_function_context(result, content, query_results)
    ...
    def _extract_related_functions(self, query_results: Dict[str, List[QueryResult]],
                                 content: str, primary_functions: List[FunctionContext]) -> List[FunctionContext]:
    ...
    def _create_function_context(self, result: QueryResult, content: str,
                               query_results: Dict[str, List[QueryResult]]) -> FunctionContext:
    ...
    def _find_calls_in_range(self, call_results: List[QueryResult],
                           start_byte: int, end_byte: int) -> List[str]:
    ...
    def _find_variables_in_range(self, var_results: List[QueryResult],
                               start_byte: int, end_byte: int) -> List[str]:
        ...
        for var in var_results:
            if start_byte <= var.start_byte <= end_byte:
                ...
                var_name = (
                    var.captures.get('assignment.target', '') or
                    var.captures.get('variable.name', '') or
                    var.captures.get('variable.usage', '')
                )
                if var_name:
    ...
    def _calculate_confidence(self, primary_functions: List[FunctionContext],
                            query_results: Dict[str, List[QueryResult]]) -> float:
    ...
    def extract_function_by_name(self, file_path: str, content: str,
                               function_name: str) -> Optional[FunctionContext]:
