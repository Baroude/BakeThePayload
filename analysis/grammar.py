...
class LanguageGrammarManager:
    ...
    def get_available_languages(self) -> List[SupportedLanguage]:
        ...
        return [
            lang
            for lang, status in self._grammar_status.items()
...
def detect_language_from_content(
    ...
    if content_lower.startswith("#!/usr/bin/env python") or "import " in content_lower:
        ...
        content_lower.startswith("#!/usr/bin/env node") or "function " in content_lower
    ):
        return SupportedLanguage.JAVASCRIPT
    elif "public class " in content_lower or "package " in content_lower:
        return SupportedLanguage.JAVA
    elif (
        content_lower.startswith("#!/usr/bin/env ruby")
        or "def " in content_lower
        and "end" in content_lower
    ):
