...
from .base import SecurityBaseModel, EcosystemEnum, validate_version_format


class VersionRange(SecurityBaseModel):
    constraint: str = Field(..., description="Version constraint (>=1.0.0, ~1.2.0, etc.)")
...
class Component(SecurityBaseModel):
    ...
    file_path: Optional[str] = Field(default=None, description="File path containing the component")
    line_range: Optional[tuple[int, int]] = Field(default=None, description="Line range (start, end)")
...
class AffectedArtifact(SecurityBaseModel):
    ...
    ecosystem: EcosystemEnum = Field(..., description="Package ecosystem")
    affected_versions: List[VersionRange] = Field(..., description="Affected version ranges")
    fixed_versions: List[str] = Field(default_factory=list, description="Fixed version numbers")
    components: List[Component] = Field(default_factory=list, description="Affected components")
