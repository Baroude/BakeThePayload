...
class RiskFactor(SecurityBaseModel):
    ...
    weight: float = Field(
        ..., description="Weight of this factor in overall assessment"
    )
    description: Optional[str] = Field(
        default=None, description="Risk factor description"
    )
...
class Impact(SecurityBaseModel):
    ...
    scope: str = Field(
        default="unchanged", description="Scope of impact (changed/unchanged)"
    )
...
class Mitigation(SecurityBaseModel):
    type: str = Field(
        ..., description="Mitigation type (patch, workaround, configuration)"
    )
...
class RiskAssessment(SecurityBaseModel):
    ...
    temporal_score: Optional[float] = Field(
        default=None, description="CVSS temporal score"
    )
    environmental_score: Optional[float] = Field(
        default=None, description="CVSS environmental score"
    )
    ...
    mitigations: List[Mitigation] = Field(
        default_factory=list, description="Available mitigations"
    )
    ...
    def calculate_overall_risk(self) -> float:
        ...
        if self.mitigations:
            mitigation_factor = sum(m.effectiveness for m in self.mitigations) / len(
                self.mitigations
            )
            risk *= (
                1.0 - mitigation_factor * 0.5
            )  # Mitigations can reduce risk by up to 50%
